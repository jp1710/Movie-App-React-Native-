...
import {addLike} from '../store/profile/like';

const TvDetails = ({navigation, route}) => {
  ...
  //const upCast = moviesData.upcomemovie.upcoming;
  //const popCast = moviesData.popCastMov.popularMoviCasting;
  const [likes, setLikes] = useState(0);
  const [series, setSeries] = useState([]);
  const casting = async () => {
    const cast = [];
    const MOVI_URL1 = 'http://47.254.174.28/series/id/' + imdb_id + '/cast/';
    for (var i = 0; i < 10; i++) {
      ...
      //console.log(roles[i].actor.imdb_id);
